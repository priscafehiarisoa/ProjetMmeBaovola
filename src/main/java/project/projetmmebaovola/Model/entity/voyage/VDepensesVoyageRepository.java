...
public interface VDepensesVoyageRepository extends JpaRepository<VDepensesVoyage, Integer> {
    ...
    @Query("select s from VDepensesVoyage s where  (s.prixUnitaireVoyage-(s.salairepersonnel+s.prixactivite))>=:limite1 and (s.prixUnitaireVoyage-(s.salairepersonnel+s.prixactivite))<=:limite2")
